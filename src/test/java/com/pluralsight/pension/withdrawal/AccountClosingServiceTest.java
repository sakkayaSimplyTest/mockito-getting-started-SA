...
class AccountClosingServiceTest {
    ...
    public static final String FIST_NAME = "John";
...
//        account.setDob(LocalDate.of(2000, 1,1));
        ...
        assertEquals(LocalDateTime.ofInstant(fixedTime, ZoneId.systemDefault()), accountClosingResponse.getProcessingDate());
    ...
    public void shouldApproveAccountClosingIfHolderReachesRetirementAgeToday() throws IOException {
        ...
        Account account = new Account();
        account.setFistName(FIST_NAME);
        account.setLastName(LAST_NAME);
        account.setTaxId(TAX_ID);
        final LocalDate dob = LocalDate.of(1954, 7, 4);
        account.setDob(dob);

//        when(backgroundCheckService.confirm(FIST_NAME, LAST_NAME, TAX_ID, dob))
//                .thenReturn(new BackgroundCheckResults("OK", 1));

        //BDD MOCKITO

        BDDMockito.given(backgroundCheckService.confirm(FIST_NAME, LAST_NAME, TAX_ID, dob))
